...
(AMD Ryzen 9 5900HS, 16 threads, `torch 2.14.0+cpu`), 5 warm-up frames, 60 measured frames,
on a generated synthetic clip. Raw rows, per-config summaries and `system.json` are under
`reports/benchmarks/`. Headline: `yolo26n + botsort @480` at 31.6 FPS / 38.4 ms p95;
`yolo26s @640` costs ~2.8× the p95. **No GPU rows exist** — no CUDA device was available.
...
(mean FPS vs p95 latency) because no labelled quality axis exists here without the licensed
dataset; the file says so in its first note. Its frontier is a single point —
`yolo26n_botsort_480` wins on both axes — and the report states that all three
...
courtvision export-validate --reference yolo26n.pt --exported yolo26n.onnx \
    --video data/clips/street.mp4 --benchmark
courtvision report
```
